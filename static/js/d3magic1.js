...
d3Magic.generateFromURI = function (uri, client_name, location_name) {
  d3.json(uri, function(error, rawData) {
        // root = basicTree.combine_tree(rawData, client_name, location_name); // transform into tree format
        // console.log(root);
        // var maxIndex = basicTree.getMaxIndex();
        // var maxIndex1 = basicTree.getMaxIndex1();
        // var totalCostSum = basicTree.totalCostSum();
        // var totalKWSum = basicTree.totalKWSum();
        // var smallArray = basicTree.getSmallArray();
    ...
    rowChart.width(800 * 0.75)
          ...
          .group(kW2)
          .colors(d3.scale.ordinal().domain(["weekday", "weekend"]).range(["#00a1e5", "#00a1e5"])) //#00a1e5 #6EDBB3 #649cd2
    ...
    */
    ...
    occupancyArray[1] = "Semi-occupied hours";
    occupancyArray[2] = "Occupied hours";
    ...
    // function occupancy (hour, WOStart, WOEnd, PWOStart, PWOEnd) {
    //   if (hour >= WOStart && hour < WOEnd) {
    //     return 1
    //   }
    //   else if (hour >= PWOStart && hour < PWOEnd) {
    //     return 0.5
    //   }
    //   else {
    //     return 0;
  ...
  TODO
  Summarize the Data
  Fix the initial color of the reset button
